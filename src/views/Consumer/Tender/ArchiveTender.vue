<template>
    <div>
        <check-permission/>
        <vue-table-card v-if="currentUser.features.view_tender" class="card bs-5" title="Archived Tender"
...
<template #tender_number="{ rowData }">
    <div class="fl-x fl-a-c">
      ...
      && rowData.status!=='Created' && (rowData.id===viewMoreId)"/>
                    ...
                    <btn v-if="currentUser.packages && currentUser.packages.export_tender_document === 'true'
      && (rowData.id===viewMoreId)" color="success"
...
export default {
    ...
    data () {
        return {
            listUrl      : urls.Consumer.Tender.archiveList,
    ...
    computed : {
        ...
        extraParams () {
            ...
            // Add project filters if they exist in the route query
            if (this.$route.query.project_id) {
                params.project = this.$route.query.project_id;
            }
            // if (this.$route.query.project_name) {
            //     params.project_name = this.$route.query.project_name;
            // }
    ...
    methods : {
        openTenderFromProject (rowData) {
            ...
            sessionStorage.setItem('parentTenderReturnPath', this.$route.fullPath);
            this.$router.push({ name : 'ArchivedAmendTenders', params : { id : rowData.id } });
        },
        tenderRevision (rowData) {
            this.$router.push('/app/tender/' + rowData.id + '/details/#Submitted');
        },
        viewDetails (rowData) {
            // Save return path with current query params
            const projectId = this.$route.query.project_id;
            const projectName = this.$route.query.project_name;
            if (projectId) {
                const returnPath = `/app/archived/tender/?project_id=${projectId}&project_name=${projectName || ''}`;
                sessionStorage.setItem('tenderReturnPath', returnPath);
            }
